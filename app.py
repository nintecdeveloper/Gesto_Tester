...
try:
    ...
    print("⚠️  [2FA] pyotp/qrcode no instal·lat. pip install pyotp qrcode per activar 2FA.")
...
class UserTOTP(db.Model):
    """Emmagatzema el secret TOTP de cada usuari per al 2FA."""
    __tablename__ = 'user_totp'
    id         = db.Column(db.Integer, primary_key=True, autoincrement=True)
    username   = db.Column(db.String(50), nullable=False, unique=True)
    secret     = db.Column(db.String(64), nullable=False)   # Base32 secret TOTP
    enabled    = db.Column(db.Boolean, default=False)        # Activat per admin/usuari
    configured = db.Column(db.Boolean, default=False)        # Usuari ha completat el setup
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

# Usuaris per als quals el 2FA és obligatori (mai es pot saltar)
MANDATORY_2FA_USERS = {'antonio', 'pau'}
...
# ═══════════════════════════════════════════════════════════════
# FLASK-LIMITER — RATE LIMITING PER A LOGIN
# ═══════════════════════════════════════════════════════════════
...
limiter = Limiter(
    ...
    default_limits=[],          # Cap límit per defecte a les altres rutes
    storage_uri="memory://",
)

# Registre d'intents fallits per IP: {ip: {"count": int, "blocked_until": datetime|None}}
_login_attempts: dict = {}
_login_lock = threading.Lock()

# Credencials dels usuaris (validació servidor — contrasenya NO s'envia al client)
_LOGIN_USERS = {
    "antonio":    {"password": "admin123",     "id": 1,  "name": "Antonio",          "role": "admin",    "email": "antonio@rodonverges.com",    "phone": "+34 93 000 00 01", "active": True,  "color": "teal",   "sede": "Mataró"},
    "pau":        {"password": "pau123",        "id": 3,  "name": "Pau",              "role": "admin",    "email": "pau@rodonverges.com",        "phone": "+34 93 000 00 03", "active": True,  "color": "orange", "sede": "Vilassar"},
    "myriam":     {"password": "Myriam123",     "id": 10, "name": "Myriam Recolons",  "role": "empleado", "email": "myriam@rodonverges.com",     "phone": "+34 93 000 00 10", "active": True,  "color": "blue",   "sede": "Mataró",   "departamento": "laboral"},
    "anna_f":     {"password": "Anna123",       "id": 11, "name": "Anna Fabregà",     "role": "empleado", "email": "anna.f@rodonverges.com",     "phone": "+34 93 000 00 11", "active": True,  "color": "purple", "sede": "Mataró",   "departamento": "fiscal"},
    "montse":     {"password": "Montse123",     "id": 12, "name": "Montse Martín",    "role": "empleado", "email": "montse@rodonverges.com",     "phone": "+34 93 000 00 12", "active": True,  "color": "pink",   "sede": "Mataró",   "departamento": "mercantil"},
    "cristina":   {"password": "Cristina123",   "id": 20, "name": "Cristina Navas",   "role": "empleado", "email": "cristina@rodonverges.com",   "phone": "+34 93 000 00 20", "active": True,  "color": "green",  "sede": "Vilassar", "departamento": "fiscal"},
    "anna_l":     {"password": "Anna123",       "id": 21, "name": "Anna Llinas",      "role": "empleado", "email": "anna.l@rodonverges.com",     "phone": "+34 93 000 00 21", "active": True,  "color": "cyan",   "sede": "Vilassar", "departamento": "laboral"},
    "anna_m":     {"password": "Anna123",       "id": 22, "name": "Anna Maria Rodón", "role": "empleado", "email": "anna.m@rodonverges.com",     "phone": "+34 93 000 00 22", "active": True,  "color": "yellow", "sede": "Vilassar", "departamento": "mercantil"},
...
def get_message_attachment(msg_id, filename):
    ...
    try:
        ...
        return jsonify({'ok': False, 'error': str(e)}), 500
# ═══════════════════════════════════════════════════════════════
# API LOGIN — Autenticació amb rate limiting per IP
# ═══════════════════════════════════════════════════════════════
...
def api_login():
    """
    Endpoint d'autenticació amb protecció de força bruta i suport 2FA:
    - Màxim 5 intents per minut per IP (Flask-Limiter)
    - Bloqueig de 15 minuts després de 5 intents fallits consecutius
    - Si el 2FA és necessari, retorna requires_2fa=True i desa l'estat a la sessió
    - Si l'usuari és obligatori (antonio/pau) però no ha configurat el 2FA,
      retorna needs_2fa_setup=True
    """
    ip = get_remote_address()
    now = datetime.utcnow()

    # — Comprova bloqueig manual (15 min per intents fallits)
    with _login_lock:
        entry = _login_attempts.get(ip)
        if entry and entry.get("blocked_until") and now < entry["blocked_until"]:
            remaining_s = int((entry["blocked_until"] - now).total_seconds())
            remaining_m = max(1, (remaining_s + 59) // 60)
            logger.warning(f"[Login] IP bloquejada: {ip} ({remaining_m} min restants)")
            return jsonify({
                "ok": False,
                "blocked": True,
                "error": f"Massa intents fallits. Accés bloquejat. Torneu-ho a provar en {remaining_m} minut{'s' if remaining_m != 1 else ''}.",
            }), 429
    ...
    if not user_rec or user_rec["password"] != password or not user_rec.get("active"):
        ...
        with _login_lock:
            ...
            if fails >= 5:
                ...
                logger.warning(f"[Login] IP bloquejada 15 min: {ip} (5 intents fallits)")
                return jsonify({
                    "ok": False,
                    "blocked": True,
                    "error": "Massa intents fallits. Accés bloquejat 15 minuts.",
                }), 429
        logger.info(f"[Login] Intent fallit #{fails} per {ip} (usuari: {username!r})")
        return jsonify({"ok": False, "error": "Usuari o contrasenya incorrectes."}), 401

    # Èxit — neteja el comptador de fallades
    with _login_lock:
        _login_attempts.pop(ip, None)

    # ── Comprovació 2FA ──────────────────────────────────────────
    if TOTP_AVAILABLE:
        is_mandatory = username in MANDATORY_2FA_USERS
        totp_rec = UserTOTP.query.filter_by(username=username).first()
        is_enabled  = totp_rec is not None and totp_rec.enabled
        is_configured = totp_rec is not None and totp_rec.configured

        # Els usuaris obligatoris o els que han activat el 2FA han de verificar-lo
        needs_2fa = is_mandatory or is_enabled

        if needs_2fa:
            if not is_configured:
                # El 2FA és necessari però encara no s'ha configurat (primer ús)
                user_data = {k: v for k, v in user_rec.items() if k != "password"}
                user_data["username"] = username
                logger.info(f"[Login] {username}: 2FA obligatori però no configurat → setup requerit")
                return jsonify({"ok": True, "needs_2fa_setup": True, "user": user_data})
            else:
                # 2FA configurat → desa l'estat a la sessió i demana el codi
                session['pending_2fa'] = username
                logger.info(f"[Login] {username}: credencials OK, esperant codi 2FA")
                return jsonify({"ok": True, "requires_2fa": True})
    # ── Fi comprovació 2FA ───────────────────────────────────────

    user_data = {k: v for k, v in user_rec.items() if k != "password"}
    user_data["username"] = username
    logger.info(f"[Login] Accés correcte: {username} des de {ip}")
    return jsonify({"ok": True, "user": user_data})


@app.errorhandler(429)
def ratelimit_handler(e):
    """Gestor global per als errors 429 de Flask-Limiter (només afecta /api/login)."""
    return jsonify({
        "ok": False,
        "blocked": True,
        "error": "Massa intents. Espereu un minut i torneu-ho a provar.",
    }), 429


# ═══════════════════════════════════════════════════════════════
# 2FA — Verificació, Configuració i Administració
# ═══════════════════════════════════════════════════════════════

@app.route('/api/verify-2fa', methods=['POST'])
def verify_2fa():
    """
    Segon pas del login: verifica el codi TOTP.
    Requereix que /api/login hagi desat 'pending_2fa' a la sessió.
    """
    if not TOTP_AVAILABLE:
        return jsonify({'ok': False, 'error': 'El mòdul 2FA no està disponible.'}), 503

    username = session.get('pending_2fa')
    if not username:
        return jsonify({'ok': False, 'error': 'No hi ha cap sessió de login pendent.'}), 400

    data = request.get_json(force=True) or {}
    code = str(data.get('code', '')).strip().replace(' ', '')

    totp_rec = UserTOTP.query.filter_by(username=username).first()
    if not totp_rec or not totp_rec.configured:
        session.pop('pending_2fa', None)
        return jsonify({'ok': False, 'error': 'El 2FA no està configurat per a aquest usuari.'}), 400

    totp = pyotp.TOTP(totp_rec.secret)
    if not totp.verify(code, valid_window=1):
        logger.warning(f"[2FA] Codi incorrecte per a {username}")
        return jsonify({'ok': False, 'error': 'Codi incorrecte. Torneu-ho a provar.'}), 401

    session.pop('pending_2fa', None)
    user_rec = _LOGIN_USERS.get(username, {})
    user_data = {k: v for k, v in user_rec.items() if k != "password"}
    user_data["username"] = username
    logger.info(f"[2FA] Verificació correcta per a {username}")
    return jsonify({'ok': True, 'user': user_data})
...
def api_2fa_setup():
    """
    Genera o recupera el secret TOTP d'un usuari i retorna l'URI otpauth://
    i el QR code en SVG per mostrar-lo a la pàgina de configuració.
    """
    if not TOTP_AVAILABLE:
        return jsonify({'ok': False, 'error': 'El mòdul 2FA no està disponible.'}), 503
    ...
    if not username or username not in _LOGIN_USERS:
        return jsonify({'ok': False, 'error': 'Usuari no vàlid.'}), 400

    totp_rec = UserTOTP.query.filter_by(username=username).first()

    # Si no té secret o no ha completat la configuració, en generem un de nou
    if not totp_rec:
        secret = pyotp.random_base32()
        totp_rec = UserTOTP(username=username, secret=secret, enabled=False, configured=False)
        db.session.add(totp_rec)
        db.session.commit()
    elif not totp_rec.configured:
        # Regenera el secret si no s'ha completat el setup anterior
        totp_rec.secret = pyotp.random_base32()
        db.session.commit()

    user_info = _LOGIN_USERS[username]
    totp = pyotp.TOTP(totp_rec.secret)
    otpauth_uri = totp.provisioning_uri(
        name=user_info.get('email', username),
        issuer_name='Gestoria Rodonvergés'
    )

    # Genera QR code com a SVG inline
    try:
        factory = qrcode.image.svg.SvgPathImage
        qr_img = qrcode.make(otpauth_uri, image_factory=factory, box_size=8)
        svg_buf = _io.BytesIO()
        qr_img.save(svg_buf)
        svg_data = svg_buf.getvalue().decode('utf-8')
    except Exception as e:
        logger.warning(f"[2FA] No s'ha pogut generar el QR SVG: {e}")
        svg_data = None
...
def api_2fa_setup_verify():
    """
    Verifica el codi TOTP introduït durant el setup per confirmar que
    l'usuari ha escanejat correctament el QR i activa el 2FA.
    """
    if not TOTP_AVAILABLE:
        return jsonify({'ok': False, 'error': 'El mòdul 2FA no està disponible.'}), 503
    ...
    totp_rec = UserTOTP.query.filter_by(username=username).first()
    if not totp_rec:
        return jsonify({'ok': False, 'error': 'El 2FA no ha estat inicialitzat. Torna a carregar la pàgina.'}), 400

    totp = pyotp.TOTP(totp_rec.secret)
    if not totp.verify(code, valid_window=1):
        return jsonify({'ok': False, 'error': 'Codi incorrecte. Comprova que l\'hora del dispositiu és correcta.'}), 401

    totp_rec.configured = True
    totp_rec.enabled = True
    db.session.commit()
    logger.info(f"[2FA] Setup completat correctament per a {username}")
    return jsonify({'ok': True, 'message': 'El 2FA s\'ha configurat correctament!'})


@app.route('/api/admin/toggle-2fa', methods=['POST'])
def admin_toggle_2fa():
    """
    Activa o desactiva el 2FA per a un usuari concret.
    Només accessible des del panell d'administració.
    """
    if not TOTP_AVAILABLE:
        return jsonify({'ok': False, 'error': 'El mòdul 2FA no està disponible.'}), 503
    ...
    if not target or target not in _LOGIN_USERS:
        return jsonify({'ok': False, 'error': 'Usuari no vàlid.'}), 400

    totp_rec = UserTOTP.query.filter_by(username=target).first()

    if not enable:
        # Desactiva el 2FA (manté el secret per si es torna a activar)
        if totp_rec:
            totp_rec.enabled = False
            totp_rec.configured = False  # Ha de tornar a fer el setup
            db.session.commit()
        logger.info(f"[Admin 2FA] 2FA desactivat per a {target}")
        return jsonify({'ok': True, 'enabled': False, 'configured': False})
    else:
        # Activa el 2FA — l'usuari haurà de configurar-lo al /setup-2fa
        if not totp_rec:
            secret = pyotp.random_base32()
            totp_rec = UserTOTP(username=target, secret=secret, enabled=True, configured=False)
            db.session.add(totp_rec)
        else:
            totp_rec.enabled = True
        db.session.commit()
        logger.info(f"[Admin 2FA] 2FA activat per a {target}")
        return jsonify({'ok': True, 'enabled': True, 'configured': totp_rec.configured})


@app.route('/api/2fa/status', methods=['GET'])
def api_2fa_status():
    """Retorna l'estat del 2FA per a tots els usuaris (per al panell d'admin)."""
    if not TOTP_AVAILABLE:
        return jsonify({'ok': True, 'available': False, 'statuses': {}})

    totp_records = UserTOTP.query.all()
    totp_map = {r.username: r for r in totp_records}
    statuses = {}
    for u in _LOGIN_USERS:
        rec = totp_map.get(u)
        statuses[u] = {
            'mandatory':  u in MANDATORY_2FA_USERS,
            'enabled':    rec.enabled    if rec else False,
            'configured': rec.configured if rec else False,
        }
    return jsonify({'ok': True, 'available': True, 'statuses': statuses})


@app.route('/setup-2fa')
def setup_2fa_page():
    """Pàgina per configurar l'autenticació de doble factor."""
...
with app.app_context():
    try:
        ...
        # Migració 2FA: inicialitzar registres obligatoris (antonio i pau) si no existeixen
        if TOTP_AVAILABLE:
            for mand_user in MANDATORY_2FA_USERS:
                if not UserTOTP.query.filter_by(username=mand_user).first():
                    secret = pyotp.random_base32()
                    db.session.add(UserTOTP(username=mand_user, secret=secret, enabled=True, configured=False))
            db.session.commit()
            logger.info("✅ Registres 2FA inicialitzats per a usuaris obligatoris")
